...
async function getViloyatSvod(date) {
    try {
        ...
        return allDistricts.map(dName => {
            ...
            const tumanRows = rawEntriesRes.rows.filter(r => r.norm_dist === normD && districtOfficialSchools.includes(normalizeKey(r.norm_school)));
            const yestRows = yestEntriesRes.rows.filter(r => r.norm_dist === normD && districtOfficialSchools.includes(normalizeKey(r.norm_school)));
...
async function getRecentActivity(limit = 20, offset = 0) {
    try {
        const rowsRes = await db.query(`SELECT * FROM attendance ORDER BY id DESC LIMIT $1 OFFSET $2`, [limit, offset]);
        const countRes = await db.query('SELECT count(*) as count FROM attendance');
